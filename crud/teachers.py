...
class TeacherCrud(__TeacherController):
    ...
    async def pull_file(self, teacher_id:str):
        try:
            result = (await self.db.execute(
                select(self.file_table.ai_response).where(self.file_table.teacher_id == teacher_id)
            )
          ).mappings().all()
            if not result:
    ...
    async def pull_image(self, teacher_id:str):
        try:
            result = (await self.db.execute(
                select(self.image_table.ai_response).where(self.image_table.teacher_id == teacher_id)
            )
          ).mappings().all()
            if not result:
